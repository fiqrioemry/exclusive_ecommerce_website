  ...
  return (
    <div className="flex flex-wrap w-full mb-[50px]">
      {products.map((product, index) => {
        const { title, thumbnail, discountPercentage, price } = product;
        return (
          <div className="lg:w-1/4 md:w-1/3 w-1/2 px-2 py-2" key={index}>
            ...
            <div className="bg-gray-200/50 rounded-md relative mb-4 max-h-[250px] group overflow-hidden">
              ...
              <button className="py-4 px-12 w-full h-[50px] bg-secondary text-white rounded-md absolute cursor-pointer group-hover:bottom-0 -bottom-14 transition-all ease-in-out duration-300">
                Add to Cart
              </button>
            </div>
            {/* details product */}
